                    ...
                    <div class="mt-4">
                        ...
                        <div class="text-95 text-secondary-d3">
                            ...
                            @foreach($order_products as $order_product)
                                @php
                                    $attrList = [];
                                    if (!empty($order_product->variants)) {
                                        $decoded = json_decode($order_product->variants, true);
                                        if (is_array($decoded)) {
                                        ...
                                        } elseif (is_string($order_product->variants) && trim($order_product->variants) !== '' && $order_product->variants !== '[]' && $order_product->variants !== '{}') {
                                            $attrList[] = e($order_product->variants);
                                        }
                                    }
                                    if (empty($attrList) && !empty($order_product->mat_language)) {
                                        $attrList[] = e($order_product->mat_language);
                                    }
                                    $attrStr = implode(' | ', $attrList);

                                    $itemImg = !empty($order_product->order_products_image) ? asset($order_product->order_products_image) : null;
                                @endphp
                                <div class="row mb-2 mb-sm-0 py-25 align-items-center">
                                    <div class="d-none d-sm-block col-1">{{ $count }}</div>
                                    <div class="col-9 col-sm-5">
                                        <div class="d-flex align-items-center">
                                            @if($itemImg)
                                                <img src="{{ $itemImg }}" class="rounded me-2 border" width="40" height="40" style="object-fit: contain; background: #fff; padding: 2px;">
                                            @endif
                                            <div>
                                                <div class="fw-bold text-dark">{{ strip_tags($order_product->order_products_name) }}</div>
                                                @if(!empty($attrStr))
                                    ...
                                    <div class="d-none d-sm-block col-2 text-95">${{ number_format($order_product->order_products_price, 2) }}</div>
                                    <div class="col-2 text-secondary-d2">${{ number_format($order_product->order_products_subtotal, 2) }}</div>
                                </div>
