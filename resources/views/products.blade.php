            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-12">
                    <div class="banner-content animate">
                        <h1><span class="blue">{{ $banner->title }}</span>{{ $banner->text2 }}</h1>
                        {!! $banner->description !!}
                ...
                <div class="col-lg-12 col-md-12 col-12">
                    <div class="seller-heading">
                        @if(request('search'))
                            ...
                            <p>Showing products matching your search term. <a href="{{ route('product') }}" class="text-decoration-underline text-primary">Clear Search</a></p>
                        @else
                            ...
                            <p>Whether you're here for a laugh, a statement, or a gift that actually gets remembered, our
                                designs are sorted so you can find the right one fast. Explore the categories below and pick the
            ...
            <div class="row">
                ...
                @forelse($product as $item)
                    @php
                        ...
                        $primaryImg = (isset($item->product_images) && count($item->product_images) > 0) ? asset($item->product_images->first()->image_path) : '';
                    @endphp
                        ...
                        <div class="product-main {{ $loop->iteration % 2 != 0 ? 'bg-blk' : '' }}">
                            ...
                            <div class="product-picture">
                                ...
                                <a href="{{ route('product_detail', $item->id) }}">
                                    ...
                                    @if (!empty($primaryImg))
                                        <img src="{{ $primaryImg }}"
                                            class="img-fluid" alt="{{ strip_tags($item->name) }}">
                                    @endif
                ...
                @empty
                    ...
                    <div class="col-12">
                        ...
                        <div class="text-center py-5">
                            <h4>No products found {{ request('search') ? 'matching "' . request('search') . '"' : '' }}.</h4>
                            @if(request('search'))
