...
@section('css')
    ...
    <style>
        ...
        .YouOrder {
            ...
            border-radius: 8px;
            margin-bottom: 20px;
            box-shadow: 0 4px 15px rgba(0, 107, 239, 0.2);
        }
                    ...
                    <li>
                        <div class="banner-content">
                            <div class="section-heading">
                                <h1 class="blue" style="color: #006bef;">Checkout</h1>
                            </div>
                        </div>
                    </li>
                    <li>
            ...
            <div class="row">
                <div class="col-lg-12 col-md-12 col-12">
                    <div class="banner-content animate">
                        <h1><span class="blue">Checkout</span></h1>
            ...
            <div class="row">
                ...
                <div class="col-md-7 col-lg-7 col-sm-7 col-xs-12">
                    ...
                    <form action="{{ route('order.place') }}" method="POST" id="order-place">
                        ...
                        @else
                            <a href="{{ url('signin') }}" class="btn btn-outline-primary mb-3">Returning customer? Click
                                here to login</a>
                            <div class="form-group mb-3">
                            ...
                            @if (!Auth::check())
                                ...
                                {{-- Password fields — sirf tab show honge jab checkbox checked ho --}}
                                <div id="password-fields"
                                    style="display: {{ !empty(old('create_account')) ? 'block' : 'none' }};">
                                    <div class="form-group mb-3">
                ...
                <div class="col-md-5 col-lg-5 col-sm-5 col-xs-12">
                    ...
                    <div class="YouOrder">
                        ...
                        @foreach ($cart as $key => $value)
                            @php
                                $itemBasePrice = $value['baseprice'] ?? ($value['price'] ?? 0);
                                $itemVarPrice = $value['variation_price'] ?? 0;
                            @endphp
                            <h5>{{ $value['name'] }} x {{ $value['qty'] }} <span
                                    class="float-right">${{ number_format($itemBasePrice * $value['qty'], 2) }}</span>
                            </h5>
                            <?php $subtotal += $itemBasePrice * $value['qty'];
                            $variation += $itemVarPrice;
                            ?>
                        @endforeach
                        <div class="amount-wrapper">
                            <h2>Item Subtotal <span>${{ number_format($subtotal, 2) }}</span></h2>
                            <h2> Variation <span>${{ number_format($variation, 2) }}</span></h2>
                                ...
                                <div class="card-body">
                                    <div class="stripe-form-wrapper require-validation"
                                        ...
                                        <div class="form-group">
                                            <button class="btn btn-stripe-pay btn-block" type="button"
                                                id="stripe-submit">Pay Now
                                                ${{ number_format($subtotal + $variation, 2) }}</button>
                                        </div>
                                    </div>
                                </div>
                            </div>
...
@section('js')
    ...
    <script>
        ...
        paypal.Button.render({
            ...
            client: {
                sandbox: '{{ env('PAYPAL_CLIENT_ID', 'AV06KMdIerC8pd6_i1gQQlyVoIwV8e_1UZaJKj9-aELaeNXIGMbdR32kDDEWS4gRsAis6SRpUVYC9Jmf') }}',
    ...
    <script>
      // ============================================
      // CREATE ACCOUNT TOGGLE — Show/Hide password fields
      // ============================================
      $(document).ready(function () {

          function togglePasswordFields() {
              if ($('#create_account').is(':checked')) {
                  $('#password-fields').slideDown(200);
                  $('#password, #confirm_password').prop('required', true);
              } else {
                  $('#password-fields').slideUp(200);
