    ...
    public function store(StoreProductRequest $request)
    {
        ...
        try {
            ...
            if ($request->has('product_attributes')) {
                foreach ($request->product_attributes as $index => $attr) {
                    $imagePath = null;
                    if ($request->hasFile("product_attributes.{$index}.image")) {
                        $imagePath = $this->uploadFile(
                            $request->file("product_attributes.{$index}.image"),
                            'uploads/variations/',
                            'variation'
                        );
                    }
    ...
    public function update(UpdateProductRequest $request, Product $product)
    {
        ...
        try {
            ...
            if ($request->hasFile('image')) {
                ...
                $oldPrimaryImage = $product->images()
                    ->where('is_primary', 1)
                    ->first();
                ...
                if ($oldPrimaryImage) {
                    ...
                    if (
                        $oldPrimaryImage->image_path &&
                        file_exists(public_path($oldPrimaryImage->image_path))
                    ) {
                        unlink(public_path($oldPrimaryImage->image_path));
                    }
            ...
            // =========================================================
            // UPDATE PRODUCT ATTRIBUTES / VARIATIONS
            // =========================================================
            ...
            if ($request->has('product_attributes')) {
                ...
                /*
            |--------------------------------------------------------------------------
            | Store old variation images before deleting records
            |--------------------------------------------------------------------------
            */
                ...
                /*
            |--------------------------------------------------------------------------
            | Keep track of images that are still being used
            |--------------------------------------------------------------------------
            */
            ...
            */
                ...
                foreach ($request->product_attributes as $index => $attr) {
                    ...
                    if (
                        $request->hasFile(
                            "product_attributes.{$index}.image"
                        )
                    ) {
                    ...
                    elseif (!empty($attr['existing_image'])) {

                        $imagePath = $attr['existing_image'];
            ...
            */
                ...
                foreach ($oldImages as $oldImage) {
                    ...
                    if (!in_array($oldImage, $usedOldImages)) {
