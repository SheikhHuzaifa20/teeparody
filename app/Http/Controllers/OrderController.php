...
use App\Models\OrderProducts as orders_products;
	...
	public function placeOrder(Request $request)
	{
		...
		foreach ($cart as $key => $value) {
			...
			if (is_array($value) && isset($value['name'])) {
				...
				$priceVal = $value['baseprice'] ?? $value['price'] ?? 0;

				$subtotal += $priceVal * ($value['qty'] ?? 1);
			}
		}
		...
		$order->order_shipping = $cart['shipping'] ?? 0;
		$order->country_code = $request->country_code;
		...
		$total = $subtotal + ($cart['shipping'] ?? 0);
		...
		// ============================================
			...
			try {
				...
				try {
					...
					return redirect()
						->back()
						->with('stripe_error', $e->getMessage());
				}
			...
			if (
				...
				// Payment successful but order is still pending
				$order->order_status = 'pending';
			}
		}
		...
		$expNum = explode('-', $record->invoice_number ?? '0');
		...
		if ($order->save()) {
			...
			foreach ($cart as $key => $value) {
				...
				if (is_array($value) && !empty($value['name'])) {
					...
					$varVal = $value['variant_price'] ?? $value['variation_price'] ?? 0;

					$order_products = new orders_products();

					$order_products->order_products_product_id = $value['id'];

					$order_products->user_id = Auth::check()
						? Auth::user()->id
						: 0;

					$order_products->order_products_name = $value['name'];

					$order_products->order_products_price = $priceVal;

					$order_products->orders_id = $orders->id;

					$order_products->order_products_qty = $value['qty'] ?? 1;

					$order_products->mat_language = $value['mat_language'] ?? '';

					$order_products->shipping = $cart['shipping'] ?? 0;

					$order_products->order_products_subtotal =
						$priceVal * ($value['qty'] ?? 1) + $varVal;

					$attrVal = $value['attributes_array'] ?? $value['attributes'] ?? $value['variation'] ?? [];
					if (is_array($attrVal)) {
						$order_products->variants = json_encode($attrVal);
					} else {
						$order_products->variants = (string)$attrVal;
					}

					$order_products->order_products_image = $value['image'] ?? null;
